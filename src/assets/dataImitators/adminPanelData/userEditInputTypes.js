export const userEditInputTypes = [
  // name
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "firstName",
    userName: "firstname",
    userLabel: "სახელი",
    error: "firstnameError",
  },
  // lastname
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "lastName",
    userName: "lastname",
    userLabel: "გვარი",
    error: "lastnameError",
  },
  // personalId
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "PersonalId",
    userName: "personalId",
    userLabel: "პირადი ნომერი",
    error: "personalIdError",
  },
  // email
  {
    xs: 12,
    sm: 4,
    inputType: "email",
    userId: "formGridEmail",
    userName: "email",
    userLabel: "ელ.ფოსტა",
    error: "emailError",
  },
  // mobile
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "mobile",
    userName: "mobile",
    userLabel: "მობილური",
    error: "mobileError",
  },
  // birthDate
  {
    xs: 12,
    sm: 4,
    inputType: "date",
    userId: "birthDate",
    userName: "birthDate",
    userLabel: "დაბადების თარიღი",
    error: "birthDayError",
  },
  // profession
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "profession",
    userName: "profession",
    userLabel: "თანამდებობა",
    error: "professionError",
  },
  // gender
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "choose-gender",
    userName: "gender",
    userLabel: "აირჩიეთ სქესი",
    error: "genderError",
    option: [
      {
        key: "male",
        name: "მამრობითოი",
      },
      {
        key: "female",
        name: "მდედრობითი",
      },
    ],
  },
  // bloodGroup
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "choose-blood-group",
    userName: "bloodGroup",
    userLabel: "სისხლის ჯგუფი",
    error: "bloodGroupError",
    option: [
      {
        key: "|",
        name: "|",
      },
      {
        key: "||",
        name: "||",
      },
      {
        key: "|||",
        name: "|||",
      },
      {
        key: "|V",
        name: "|V",
      },
    ],
  },
  // bloodBandar
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "choose-blood-bandar",
    userName: "bloodBandar",
    userLabel: "სისხლის რეზუსი",
    error: "bloodBandarError",
    option: [
      {
        key: "positive ",
        name: "დადებითი",
      },
      {
        key: "negative",
        name: "უარყოფითი",
      },
    ],
  },
  // allergy
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "allergy",
    userName: "allergy",
    userLabel: "ალერგიები",
    error: "allergyError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // smok4r
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "smoker",
    userName: "smoker",
    userLabel: "მწეველი",
    error: "smokerError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // previousConvictions
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "previousConvictions",
    userName: "previousConvictions",
    userLabel: "ნასამართლევი",
    error: "previousConvictionsError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // married
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "married",
    userName: "married",
    userLabel: "ნასამართლევი",
    error: "marriedError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // secondMobile
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "secondMobile",
    userName: "secondMobile",
    userLabel: "მეორე ნომერი",
    error: "secondMobileError",
  },
  // relativeMobile
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "relativeMobile",
    userName: "relativeMobile",
    userLabel: "ნათესავის ნომერი",
    error: "relativeMobileError",
  },

  // secondEmail
  {
    xs: 12,
    sm: 4,
    inputType: "email",
    userId: "secondEmail",
    userName: "secondEmail",
    userLabel: "მეორე ელ.ფოსტა",
    error: "secondEmailError",
  },
  // firstAddress
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "firstAddress",
    userName: "firstAddress",
    userLabel: "იურიდიული მისამართი",
    error: "firstAddressError",
  },

  // secondAddress
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "secondAddress",
    userName: "secondAddress",
    userLabel: "ფაქტიური მისამართი",
    error: "secondAddressError",
  },
  // bankName
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "bankName",
    userName: "bankName",
    userLabel: "ბანკის დასახელება",
    error: "bankNameError",
  },
  // bankCode
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "bankCode",
    userName: "bankCode",
    userLabel: "ბანკის კოდი",
    error: "bankCodeError",
  },
  // bankAccount
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "bankAccount",
    userName: "bankAccount",
    userLabel: "ანგარიშის ნომერი",
    error: "bankAccountError",
  },

  // workAddress
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "workAddress",
    userName: "workAddress",
    userLabel: "სამსახურის მისამართი",
    error: "workAddressError",
  },
  // workZip
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "workZip",
    userName: "workZip",
    userLabel: "სამსახურის საფოსტო კოდი",
    error: "workZipError",
  },
  // flexitime
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "flexitime",
    userName: "flexitime",
    userLabel: "თავისუფალი გრაფიკი",
    error: "flexitimeError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // salaryPeriod
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "salaryPeriod",
    userName: "salaryPeriod",
    userLabel: "ხელფასის პერიოდულობა",
    error: "salaryPeriodError",
    option: [
      {
        key: "yes",
        name: "დიახ",
      },
      {
        key: "no",
        name: "არა",
      },
    ],
  },
  // salary
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "salary",
    userName: "salary",
    userLabel: "დარიცხული ხელფასი",
    error: "salaryError",
  },
  // overtimeSalary
  {
    xs: 12,
    sm: 4,
    inputType: "text",
    userId: "overtimeSalary",
    userName: "overtimeSalary",
    userLabel: "ზეგანაკვეთური(1სთ)",
    error: "overtimeSalaryError",
  },
  // currency
  {
    xs: 12,
    sm: 4,
    inputType: "select",
    userId: "currency",
    userName: "currency",
    userLabel: "ვალუტა",
    error: "currencyError",
    option: [
      {
        key: "GEL",
        name: "GEL",
      },
      {
        key: "USD",
        name: "USD",
      },
      {
        key: "EUR",
        name: "EUR",
      },
    ],
  },
  // startWorking
  {
    xs: 12,
    sm: 4,
    inputType: "date",
    userId: "startWorking",
    userName: "startWorking",
    userLabel: "სამუშაოუს დაწყება",
    error: "startWorkingError",
  },
];
